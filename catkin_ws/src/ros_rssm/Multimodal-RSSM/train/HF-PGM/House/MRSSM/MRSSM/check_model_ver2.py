...
def plot_rcon(t):
    ...
    if cfg.env.action_name=="action":
        ax5.imshow(img_map)
        ax5.set_title("Action (Dummy)")

    elif cfg.env.action_name=="Pose":
        Pose_list=np.append(Pose_list,display_pose(actions[t][0].detach().cpu().numpy()),axis=0)
        ...
        #位置尤度の出力
        ax5.scatter(pose_predict_loc[t,0,0]*20+190,pose_predict_loc[t,0,1]*(-20)+115, s = 5, c="green")
