...
from utils.models.encoder import ObsEncoder, MultimodalObsEncoder, StochasticStateModel
from utils.dist import get_poe_state, get_mopoe_state
...
class MultimodalTransitionModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.obs_encoder = MultimodalObsEncoder(h_size=belief_size, s_size=state_size, activation=self.act_fn, embedding_sizes=embedding_sizes, hidden_size=hidden_size, min_std_dev=self.min_std_dev, device=device)
        self.modules += self.obs_encoder.modules
        
        self.fusion_method = fusion_method
        self.num_components = num_components
        if "MoPoE" in self.fusion_method:
    ...
    def forward(self, prev_state: torch.Tensor, actions: torch.Tensor, prev_belief: torch.Tensor, observations: Optional[torch.Tensor] = None, nonterminals: Optional[torch.Tensor] = None, det=False) -> List[torch.Tensor]:
        ...
        for t in range(T - 1):
            ...
            if observations is not None:
                ...
                posterior_state, posterior_mean, posterior_std_dev = self._get_posterior_states(experts_loc, 
                                                                                                experts_scale, 
...
class MultimodalTransitionModel_emb(nn.Module):
    __constants__ = ['min_std_dev']
