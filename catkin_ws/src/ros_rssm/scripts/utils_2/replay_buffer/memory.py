...
def check_relative_path(path_name):
    if path_name[0] == ".":
        return True
    else:
        return False
...
def _load_dataset(cfg, cwd, dataset_path, D):
    if check_relative_path(dataset_path):
...
class ExperienceReplay_Multimodal:
    ...
    def _get_observation_batch(self, vec_idxs, n, L):
        pca_rand = None
        observations = dict()
        for name in self.observation_names:
    ...
    def _retrieve_batch(self, idxs, n, L, sample_for_contrastive_learning=False):
        vec_idxs = idxs.transpose().reshape(-1)  # Unroll indices
        
        observations = self._get_observation_batch(vec_idxs, n, L)
        ...
        if sample_for_contrastive_learning:
            observations4cl = self._get_observation_batch(vec_idxs, n, L)
        else:
            observations4cl = None
            
        return observations, actions, rewards, nonterminals, observations4cl

    # Returns a batch of sequence chunks uniformly sampled from the memory
    def sample(self, n, L, sample_for_contrastive_learning=False):
        batch = self._retrieve_batch(np.asarray(
            [self._sample_idx(L) for _ in range(n)]), n, L, sample_for_contrastive_learning)
