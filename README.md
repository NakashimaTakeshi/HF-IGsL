...
Please report any issue or future work by using the GitLab issue tracker: [https://gitlab.com/emlab/RULO/issues](https://gitlab.com/emlab/RULO/issues).
...
> **Note:**
  The whole development environment of the RULO project is containerized/virtualized using [Docker](https://www.docker.com/).
  This ensures that all the contributors work in the exact same software environment.

> **Note:**
  The RULO robot and simulator interfaces are primarily implemented with [ROS](http://www.ros.org/).

> **Note:**
  The RULO project relies heavily on both hardware 3D graphic acceleration and Nvidia [CUDA](https://developer.nvidia.com/cuda-toolkit), thus a discrete Nvidia GPU is highly recommended although Intel CPU graphic acceleration is also supported.
...
*   Access to developer privileges on the GitLab project at [https://gitlab.com/emlab/RULO](https://gitlab.com/emlab/RULO).

**Recommended:**

*   A [Robotis TurtleBot3](https://emanual.robotis.com/docs/en/platform/turtlebot3/overview/) robot for full operability.
    If not, the RULO simulator provides support for basic operations.
...
> **Note:**
  ...
  However, if your system has already all the necessary tools installed, you can directly set up your local user environment with `cd ~/RULO/ && bash ./INCL-USER-ENV.bash` which does not require `sudo` permissions.
...
1.   Build the Docker image:
     ...
     This script builds the image following the instructions found in `~/RULO/docker/rulo-devel/Dockerfile`.
     Enter your GitLab developer credentials if prompted.

> **Note:**
  The script `BUILD-DOCKER-IMAGE.bash` first tries to download the image from the [container registry](https://gitlab.com/emlab/RULO/container_registry) of the project.
...
1.   Run the Docker container:
     ...
     This script creates or updates the container following the instructions found in `~/RULO/docker/docker-compose.yml`.
...
> **Note:**
  ...
  In this case, most 3D accelerated tools, including the RULO simulator, will be extremely slow to run.

> **Note:**
  The script `RUN-DOCKER-CONTAINER.bash` will try to resolve host name `turtlebot3.local` of the RULO robot and add its IP address to `/etc/hosts`.
  You can confirm the result with `ping turtlebot3.local` from inside the container.
  Note that you can ignore the errors if you do not plan to use the RULO robot at that time.
...
4.   Launch a couple of publisher and subscriber test nodes from the `rulo_launch` package:
...
These advanced functions will help you increase both the convenience and the quality of your work for the RULO project.

#### Custom Bash Functions

The development environment contains the several useful Bash functions, all starting with the prefix `rulo-`, to make your work more convenient.
...
> **Note:**
  These Bash functions are based on helper scripts that can be found in `/root/RULO/docker/rulo-devel/scripts/` in the Docker container or in `~/RULO/docker/rulo-devel/scripts/` in the host machine.
...
This script opens [Terminator](https://gnometerminator.blogspot.com/) with the default layout configuration stored in `~/RULO/terminator/config`.
...
*   `~/RULO/docker/rulo-devel/Dockerfile`: Used by `BUILD-DOCKER-IMAGE.bash` to create the image of the shared development environment.
    It mainly describes the installations of the project tools/dependencies required inside the container.
*   `~/RULO/docker/docker-compose.yml`: Describes the interface between the host machine and the container.
...
*   `~/RULO/docker/rulo-devel/scripts/initialize-docker-container.bash`: A startup script referenced in `docker-compose.yml` and thus also invoked when (re)starting the container, most frequently the first time that `RUN-DOCKER-CONTAINER.bash` is run after the container is either stopped (host reboot) or rebuilt.
...
*   `~/RULO/docker/rulo-devel/scripts/initialize-bash-shell.bash`: A shell script appended to `/root/.bashrc` at the end of the `Dockerfile` to set up the Bash shell environment inside the container (terminal colors, shell functions, symbolic links, binary paths, environment sourcing, etc.).
...
> **Note:**
  ...
  Instead, you need to provide a link to download all your necessary large binary files from the cloud with `rulo-download-model-data`.
