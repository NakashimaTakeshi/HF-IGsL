...
This project aims to develop a shared platform for collabrative resesearch and development on the TurtleBot3 robot at [Panasonic Corporation](https://www.panasonic.com/global/home.html) and [Ritsumeikan Global Innovation Research Organization (R-GIRO)](http://en.ritsumei.ac.jp/research/r-giro/) by providing a robust, scalable, virtualized, and documented environment to all contributors.
...
The development environment contains the several useful Bash functions, all starting with the prefix `turtlebot3-`, to make your work more convenient.
Including, but not limited to:

*   `turtlebot3-download-model-data`: Download from the cloud all the large binary files required at runtime (models, datasets, dictionaries, weights, etc.).
*   `turtlebot3-initialize-catkin-workspace`: Initialize, build, and source the Catkin workspace on top of the system ROS environment.
*   `turtlebot3-reset-catkin-workspace`: Remove built artifacts, then cleanly rebuild and source the Catkin workspace on top of the system ROS environment (to use after, for example, switching branches).
*   `turtlebot3-fix-permission-issues`: Fix the various permission issues that may appear when manipulating, on the host machine, files generated by the `root` user of the Docker container.
*   `turtlebot3-get-fully-started`: Execute several of the aforementioned functions to quickly get started when entering a freshly built Docker container.
