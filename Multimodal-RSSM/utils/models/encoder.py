...
class MultimodalObsEncoder:
    ...
    def __init__(self, h_size: int, s_size: int, activation: nn.Module, embedding_sizes: Dict, hidden_size: int, min_std_dev: float, device):
        self.modules = []
    ...
    def __init__(self, embbed_size = 250, channels_base=128):
        ...
        self.down_conversion = nn.Sequential(
            nn.Conv1d(in_channels=self.conversion_channels,
                      ...
                      padding=0,
                      bias=False),
            nn.BatchNorm1d(num_features=int(embbed_size/2), affine=True, track_running_stats=True),
            nn.GLU(dim=1),
        )
...
class MultimodalEncoderNN:
    def __init__(self, 
        ...
        self.mixer = Mixer(total_embedding_size, embedding_size["fusion"], activation_function["fusion"]).to(device)
        self.modules = self.multimodal_encoder.modules + self.mixer.modules
...
class MultimodalObservationEncoder:
    ...
    def __init__(self, 
        ...
        _embedding_size = dict()
        for name in self.embedding_size:
            _embedding_size[name] = embedding_size["fusion"]
        for name in self.observation_names_enc:
            observation_shape = observation_shapes[name]
            self.encoders[name] = build_Encoder(name, observation_shapes, _embedding_size, activation_function, normalization).to(device)
            self.obs_encoder[name] = ObsEncoder_without_ht(s_size=embedding_size["fusion"], activation=getattr(F, activation_function["dense"]), embedding_size=embedding_size["fusion"], hidden_size=embedding_size["fusion"], min_std_dev=min_std_dev).to(device)
            self.modules += self.encoders[name].modules
            self.modules += self.obs_encoder[name].modules
