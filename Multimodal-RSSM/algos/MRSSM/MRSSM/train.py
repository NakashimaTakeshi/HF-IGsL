...
def train(cfg, cwd, results_dir, device):
    ...
    if cfg.train.pretrained_model != None:
        for model_name in cfg.train.pretrained_model.keys():
            model_path = os.path.join(cwd, cfg.train.pretrained_model[model_name]["model_path"])
            print("Load pretrain model ({}) from {}.".format(model_name, model_path))
            model_dicts = model._load_model_dicts(model_path)
            for obs_name in cfg.train.pretrained_model[model_name]["observations"].keys():
                if cfg.train.pretrained_model[model_name]["observations"][obs_name]["load_encoder"]:
                    model.load_encoder_state_dict(obs_name, model_dicts)
                    print("\t Load {} encoder.".foarmat(obs_name))
                if cfg.train.pretrained_model[model_name]["observations"][obs_name]["load_decoder"]:
                    model.load_decoder_state_dict(obs_name, model_dicts)
                    print("\t Load {} decoder.".foarmat(obs_name))
