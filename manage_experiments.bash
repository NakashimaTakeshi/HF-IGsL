...
# RESULT_PATH=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )'/catkin_ws/result/eval/movie/tmp/'
MODEL_PATH=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )'/catkin_ws/src/ros_rssm/scripts/'
VALDATA_PATH='dataset'
# VALDATA_PATH=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )'/dataset'
ANALYSIS_FILEPATH=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )'/catkin_ws/utils/'

echo ${MODEL_PATH}
echo ${VALDATA_PATH}

# RSSM_MODEL='RSSM_node_MRSSM.py' #RSSM_node.py
# INTEGRATION_MODE='1.0'
